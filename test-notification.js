...
async function testNotifications() {
  try {
    ...
    const existingNotifs = await Notification.find({ recipient: admin._id })
      .sort({ createdAt: -1 })
      .limit(5);
    console.log(`Found ${existingNotifs.length} notifications:`);
    existingNotifs.forEach((n) => {
      console.log(
        `  - ${n.type}: ${n.title} (read: ${n.isRead}) - ${n.createdAt}`
      );
    });
    ...
    allRecent.forEach((n) => {
      console.log(
        `  - ${n.type} → ${n.recipient?.name || "Unknown"} (${
          n.recipient?._id
        }) | Read: ${n.isRead} | ${n.createdAt}`
      );
    });
    ...
    const apiQuery = await Notification.find({
      recipient: admin._id,
      isRead: false,
    })
