...
async function checkAdmittedDates() {
  try {
    ...
    console.log('Lead ID'.padEnd(20), 'Name'.padEnd(20), 'Admitted At'.padEnd(30), 'Course'.padEnd(30), 'Assigned To');
    ...
    for (const lead of admittedLeads) {
      ...
      const localStr = admittedDate ? admittedDate.toLocaleString('en-US', { 
        year: 'numeric', 
        month: '2-digit', 
        day: '2-digit',
        hour: '2-digit',
    ...
    if (decLeads.length > 0) {
      ...
      for (const lead of decLeads) {
        console.log(`  - ${lead.leadId}: ${lead.name} - Admitted: ${lead.admittedAt}`);
      }
    }
